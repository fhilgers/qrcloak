...
use uniffi::{
    ...
    FfiConverterArc, Lift, LiftRef, LiftReturn, Lower, LowerReturn, MetadataBuffer, Result,
    RustBuffer,
};
...
pub mod objsafe {
    ...
    use dyn_clone::DynClone;
    use uniffi::{MetadataBuffer, RustBuffer};

    use crate::{any::Any, objsafe::lift::erase};
    ...
    pub trait Lift: lift::Sealed {
        fn try_lift(&self, v: Out) -> Result<Out, ()>;
        fn try_read(&self, buf: &mut &[u8]) -> Result<Out, ()>;
    }

    pub trait Lower: lower::Sealed {
        fn lower(self: Arc<Self>) -> Out;
        fn write(self: Arc<Self>, buf: &mut Vec<u8>);
    }

    impl dyn Lift {
        pub fn erase<L>() -> impl Lift
        where
            L: uniffi::Lift<crate::UniFfiTag>,
        {
            lift::erase::Lift::<L>::new()
        }
    }

    impl dyn Lower {
        pub fn erase<L>(lower: L) -> impl Lower
        where
            L: uniffi::Lower<crate::UniFfiTag>,
        {
            lower::erase::Lower::<L>::new(lower)
        }
    }

    impl<L> Lift for lift::erase::Lift<L>
    where
        L: uniffi::Lift<crate::UniFfiTag>,
    {
        fn try_lift(&self, v: Out) -> Result<Out, ()> {
            L::try_lift(unsafe { v.take() })
                .map_err(|e| ())
                .map(|v| unsafe { Out::new(v) })
        }
    ...
    impl<L> Lower for lower::erase::Lower<L>
    where
        L: uniffi::Lower<crate::UniFfiTag>,
    {
        fn lower(self: Arc<Self>) -> Out {
            unsafe { Out::new(L::lower(self.clone())) }
        }

        fn write(self: Arc<Self>, buf: &mut Vec<u8>) {
            let v: L = unsafe { self.lower().take() };
            L::write(v, buf)
        }
    }

    impl<L> lift::Sealed for lift::erase::Lift<L> where L: uniffi::Lift<crate::UniFfiTag> {}
    impl<L> lower::Sealed for lower::erase::Lower<L> where L: uniffi::Lower<crate::UniFfiTag> {}

    unsafe impl uniffi::Lift<crate::UniFfiTag> for Arc<(dyn Lift + Send + Sync + 'static)> {
        type FfiType = RustBuffer;

        fn try_lift(v: Self::FfiType) -> uniffi::Result<Self> {
            let x = lift::erase::Lift::<Self>::new();

            x.try_lift(unsafe { Out::new(v) })
                .map_err(|e| uniffi::deps::anyhow::anyhow!("got err"))
                .map(|v| unsafe { Out::take(v) })
        }
        ...
        const TYPE_ID_META: uniffi::MetadataBuffer = MetadataBuffer::from_code(4);
    }

    trait arst: Lower + DynClone {}

    dyn_clone::clone_trait_object!(arst);

    unsafe impl uniffi::Lower<crate::UniFfiTag> for Arc<(dyn Lower + Send + Sync + 'static)> {
        type FfiType = RustBuffer;

        const TYPE_ID_META: uniffi::MetadataBuffer = MetadataBuffer::from_code(4);

        fn lower(obj: Self) -> Self::FfiType {
            unsafe { Out::take(obj.lower()) }
        }
    ...
    mod lower {
        ...
        pub mod erase {
            ...
            impl<L> Lower<L> {
                pub(crate) fn new(lower: L) -> Self {
                    Self {
                        state: Some(Arc::new(lower)),
                    }
                }

                pub(crate) fn as_ref(&self) -> &L {
    ...
    mod lift {
        ...
        pub mod erase {
            ...
            pub struct Lift<L> {
                _marker: PhantomData<L>, //state: Option<L>
            }
...
struct Wrapper {
    lower: Option<Arc<dyn objsafe::Lower + Send + Sync + 'static>>,
    lift: Option<Arc<dyn objsafe::Lift + Send + Sync + 'static>>,
}
...
unsafe impl uniffi::Lower<crate::UniFfiTag> for Wrapper {
    type FfiType = <Arc<dyn objsafe::Lower + Send + Sync + 'static> as uniffi::Lower<
        crate::UniFfiTag,
    >>::FfiType;

    fn lower(obj: Self) -> Self::FfiType {
        <Arc<dyn objsafe::Lower + Send + Sync + 'static> as uniffi::Lower<crate::UniFfiTag>>::lower(
            obj.lower.clone().unwrap(),
        )
    }

    fn write(obj: Self, buf: &mut Vec<u8>) {
        <Arc<dyn objsafe::Lower + Send + Sync + 'static> as uniffi::Lower<crate::UniFfiTag>>::write(
            obj.lower.clone().unwrap(),
            buf,
        )
    }

    const TYPE_ID_META: MetadataBuffer = <Arc<dyn objsafe::Lower + Send + Sync + 'static> as uniffi::Lower<crate::UniFfiTag>>::TYPE_ID_META;
}

unsafe impl uniffi::Lift<crate::UniFfiTag> for Wrapper {
    type FfiType =
        <Arc<dyn objsafe::Lift + Send + Sync + 'static> as uniffi::Lift<crate::UniFfiTag>>::FfiType;

    fn try_lift(v: Self::FfiType) -> Result<Self> {
        Ok(Wrapper {
            lift: Some(
                <Arc<dyn objsafe::Lift + Send + Sync + 'static> as uniffi::Lift<
                    crate::UniFfiTag,
                >>::try_lift(v)?,
            ),
            lower: None,
    ...
    const TYPE_ID_META: MetadataBuffer = <Arc<dyn objsafe::Lift + Send + Sync + 'static> as uniffi::Lift<crate::UniFfiTag>>::TYPE_ID_META;
}

unsafe impl uniffi::FfiConverter<crate::UniFfiTag> for Wrapper {
    type FfiType = <Self as Lower<crate::UniFfiTag>>::FfiType;

    fn write(obj: Self, buf: &mut Vec<u8>) {
        <Self as Lower<crate::UniFfiTag>>::write(obj, buf)
    }

    fn lower(obj: Self) -> Self::FfiType {
        <Self as Lower<crate::UniFfiTag>>::lower(obj)
    }

    fn try_lift(v: Self::FfiType) -> Result<Self> {
        <Self as Lift<crate::UniFfiTag>>::try_lift(v)
    }

    fn try_read(buf: &mut &[u8]) -> Result<Self> {
        <Self as Lift<crate::UniFfiTag>>::try_read(buf)
    }

    const TYPE_ID_META: MetadataBuffer = <Self as Lower<crate::UniFfiTag>>::TYPE_ID_META;
}
...
pub extern "C" fn uniffi_qrypt_uniffi_fn_method_immutablewrapper_poll(
    ...
    let uniffi_lift_args = move || {
        Ok((
            match <::std::sync::Arc<ImmutableWrapper<Wrapper>> as ::uniffi::Lift<
                crate::UniFfiTag,
    ...
    match uniffi_lift_args() {
        ...
        Err((arg_name, anyhow_error)) => ::uniffi::rust_future_new(
            async move {
                <Option<Wrapper> as ::uniffi::LowerReturn<crate::UniFfiTag>>::handle_failed_lift(
                    arg_name,
                    anyhow_error,
                )
            },
...
const UNIFFI_META_CONST_QRYPT_UNIFFI_METHOD_IMMUTABLEWRAPPER_POLL: ::uniffi::MetadataBuffer =
    ::uniffi::MetadataBuffer::from_code(::uniffi::metadata::codes::METHOD)
        ...
        .concat_bool(true)
        .concat_value(0u8)
        .concat(<Option<Wrapper> as ::uniffi::LowerReturn<crate::UniFfiTag>>::TYPE_ID_META)
        .concat_long_str("");
#[no_mangle]
#[doc(hidden)]
pub static UNIFFI_META_QRYPT_UNIFFI_METHOD_IMMUTABLEWRAPPER_POLL: [u8;
    UNIFFI_META_CONST_QRYPT_UNIFFI_METHOD_IMMUTABLEWRAPPER_POLL.size] =
    UNIFFI_META_CONST_QRYPT_UNIFFI_METHOD_IMMUTABLEWRAPPER_POLL.into_array();
#[doc(hidden)]
#[no_mangle]
pub extern "C" fn uniffi_qrypt_uniffi_checksum_method_immutablewrapper_poll() -> u16 {
    UNIFFI_META_CONST_QRYPT_UNIFFI_METHOD_IMMUTABLEWRAPPER_POLL.checksum()
}
...
impl<T: Clone> UniffiCustomTypeConverter for Immutable<T> {
